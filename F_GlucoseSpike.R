...
GlucoseSpike = function( daysAZ, data, baseGlu, IncCut ) {
    ...
    data_tmp$x = NA
    data_tmp$x = as.numeric(rownames(data_tmp))
    data_tmp$turnPt = NA 
    for ( d in 1:length(daysAZ) ) {
        ...
        while(1) {
            ...
            pred.tmp = try(predict(fit.tmp,data_tmp[which(data_tmp$date==daysAZ[d]),]$x),silent=T)
        ...
        data_tmp$turnPt[setdiff(data_tmp$x[c(FALSE,diff(diff(predict(fit.tmp,data_tmp$x))>0)!=0)],NA)] = 1
    }

    # 그래프 # 
    # fit.tmp = loess(max_glucose ~ x, data=data_tmp[!is.na(data_tmp$max_glucose),],span=0.01,control=loess.control(surface='interpolate'))
    # x11();plot(data_tmp$x,data_tmp$max_glucose,type='l')
    # lines(data_tmp$x,predict(fit.tmp,data_tmp$x),col='red')
    # points(data_tmp$x[c(FALSE,diff(diff(predict(fit.tmp,data_tmp$x))>0)!=0)],predict(fit.tmp,data_tmp$x)[c(FALSE,diff(diff(predict(fit.tmp,data_tmp$x))>0)!=0)],col='blue')
