...
dailyLog_Record_onW = function( daysAZ, data, stat, baseGlu, FPG.L=FPG.L, PPG.U=PPG.U ) {
    ...
    for ( d in 1:length(daysAZ) ) {
        ...
        out.Plot[[d]] = ggplot(data_tmp)+ 
            ...
            geom_hline(yintercept=baseGlu,color='#40ac49',size=0.3)+
            annotation_custom(grob=textGrob(baseGlu,gp=gpar(fontsize=11,col='#231f20',fontfamily='NotoSansCJKkrR',fontface='bold')),xmin=(xlim.tmp[1]-2.5*60*60),xmax=(xlim.tmp[1]-2.5*60*60),ymin=baseGlu,ymax=baseGlu)+
            annotation_custom(grob=linesGrob(gp=gpar(col='#40ac49',lwd=1)),xmin=(xlim.tmp[1]-1.8*60*60),xmax=xlim.tmp[1],ymin=baseGlu,ymax=baseGlu)
